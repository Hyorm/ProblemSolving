...
void ft(int to, int from, int dx1, int dx2, int dy1, int dy2){
	for(int i = 0; i < 3; i++){
		if(dx1==-1){
			if(dx2==-1)C[to][i][dy1] = C[from][i][dy2];
			else C[to][i][dy1] = C[from][dx2][i];
		}else{
			if(dx2 == -1)C[to][dx1][i] = C[from][i][dy2];
			else C[to][dx1][i] = C[from][dx2][i];
		}
	}
}
...
void chg(int c1, int c2, int c3, int c4, int dx1, int dx2, int dx3, int dx4, int dy1, int dy2, int dy3, int dy4){
	...
	for(int i = 0; i < 3; i++){
		if(dx1==-1)C[c1][i][dy1] = tmp[i];
		else C[c1][dx1][i] = tmp[i];
	}
}
